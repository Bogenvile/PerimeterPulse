...
import { getRouterParam, getQuery, createError } sendError } from "nitro/h3";
...
export default defineHandler(async (event) => {
  try {
    ...
    // Ensure the error is properly thrown so Nitro handles it
    if (error && typeof error === 'object' && 'statusCode' in error) {
      throw error;
    }
    console.error("errors.get.ts error:", error);
